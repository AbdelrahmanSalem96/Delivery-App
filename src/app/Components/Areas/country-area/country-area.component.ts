...
@Component({
  ...
  styleUrl: './country-area.component.css'
})
export class CountryAreaComponent {
    ...
    if(addForm.valid){
      if(this.area.name.trim().length == InitialConstants.DefaultInt){
        ...
        if(this.area.id != InitialConstants.DefaultString){
          ...
          this.createArea.areaType=AreaTypeEnum.Country;
          this.createArea.parentId=null;
  ...
  onOpenDialog(id:string,name:string) {
    ...
    dialogConfig.data = {message:"Do You Want To Delete Country "+ name + " ? "};
  ...
  onSearch(){
    ...
    this.searchObj.Value=AreaTypeEnum.Country.toString();
