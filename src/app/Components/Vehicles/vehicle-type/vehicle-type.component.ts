...
import {CreateVehicleTypeModel,UpdateVehicleTypeModel,DeleteVehicleTypeModel,VehicleTypeDtoModel} from '../../../Models/VehicleType/VehicleType.Model';
...
export class VehicleTypeComponent {
  ...
  vehicleType:VehicleTypeDtoModel =new VehicleTypeDtoModel();
  createvehicletype:CreateVehicleTypeModel=new CreateVehicleTypeModel();
  updatevehicletype:UpdateVehicleTypeModel=new UpdateVehicleTypeModel();
  deletevehicletype:DeleteVehicleTypeModel=new DeleteVehicleTypeModel();
  ...
  previousdata:PaggedResult<VehicleTypeDtoModel>=new PaggedResult<VehicleTypeDtoModel>();

  searchObj :SearchObj= new SearchObj();
  namesearchfield:string=InitialConstants.DefaultString;
  searchfieldsfilter:PaggedCriteriaSearch<VehicleTypeDtoModel> = new PaggedCriteriaSearch<VehicleTypeDtoModel>();
  ...
  onSubmit(addForm:NgForm){
    ...
    if(addForm.valid){
      if(this.vehicleType.name.trim().length == InitialConstants.DefaultInt){
        ...
        if(this.vehicleType.id != InitialConstants.DefaultString){
          this.updatevehicletype.id=this.vehicleType.id;
          this.updatevehicletype.name=this.vehicleType.name;
          this.updatevehicletype.lastUpdatedById=this.currentUser;
          this._dataServiceService.vehicleTypeService.update(this.updatevehicletype).subscribe((res)=>{
            this._dataServiceService.spinnerService.hide();
          ...
          this.createvehicletype.name=this.vehicleType.name;
          this.createvehicletype.createdById=this.currentUser;
          this._dataServiceService.vehicleTypeService.create(this.createvehicletype).subscribe((res)=>{
            this._dataServiceService.spinnerService.hide();
            if(res.statusCode==HttpStatusCodes.OK){
  ...
  onEdit(id:string){
    this._dataServiceService.spinnerService.show();
    this._dataServiceService.vehicleTypeService.getById(id).subscribe(res => {
      this._dataServiceService.spinnerService.hide();
      if(res.statusCode==HttpStatusCodes.OK){
        this.onResetPage(InitialConstants.DefaultHidden,TabsValueEnum.Save,TabsTextEnum.Edit);
        this.vehicleType=res.data ?? new VehicleTypeDtoModel();
  ...
  onDelete(id:string){
    this._dataServiceService.spinnerService.show();
    this.deletevehicletype.id=id;
    this.deletevehicletype.deletedById=this.currentUser;
    this._dataServiceService.vehicleTypeService.delete(this.deletevehicletype).subscribe((res)=>{
      this._dataServiceService.spinnerService.hide();
      if(res.statusCode==HttpStatusCodes.OK){
  ...
  onOpenDialog(id:string,name:string) {
    ...
    dialogConfig.data = {message:"Do You Want To Delete Vehicle Type "+ name + " ? "};
    dialogConfig.width = '350px';
    dialogConfig.enterAnimationDuration='1500ms';
  ...
  onResetPage(btn:boolean=InitialConstants.DefaultShow,tabactive:TabsValueEnum=TabsValueEnum.Save,tabname:TabsTextEnum=TabsTextEnum.Save){
    this.vehicleType=new VehicleTypeDtoModel();
    this.createvehicletype=new CreateVehicleTypeModel();
    this.updatevehicletype=new UpdateVehicleTypeModel();
    this.deletevehicletype=new DeleteVehicleTypeModel();
  ...
  onLoadPageSearch(){
    this._dataServiceService.spinnerService.show();
    this._dataServiceService.vehicleTypeService.getPaggedResult(this.searchfieldsfilter).subscribe((res)=>{
      this._dataServiceService.spinnerService.hide();
      if(res.statusCode == HttpStatusCodes.OK){
        this.previousdata=res.data ?? new PaggedResult<VehicleTypeDtoModel>();
        this.totalpages=PagingInfo.GetTotalPages(res.data?.pagerInfo.totalNumberOfPages ?? InitialConstants.DefaultNoPages);
      }else if (res.statusCode == HttpStatusCodes.NotFound){
        this.previousdata=new PaggedResult<VehicleTypeDtoModel>();
        this.totalpages=[];
      }else{
        this._dataServiceService.notificationService.showErrorNotification(res.message);
        this.previousdata=new PaggedResult<VehicleTypeDtoModel>();
