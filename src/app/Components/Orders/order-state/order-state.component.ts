...
import {CreateOrderStateModel,UpdateOrderStateModel,DeleteOrderStateModel,OrderStateDtoModel} from '../../../Models/OrderState/OrderState.Model';
...
export class OrderStateComponent {
  ...
  orderState:OrderStateDtoModel =new OrderStateDtoModel();
  createOrderState:CreateOrderStateModel=new CreateOrderStateModel();
  updateOrderState:UpdateOrderStateModel=new UpdateOrderStateModel();
  deleteOrderState:DeleteOrderStateModel=new DeleteOrderStateModel();
  ...
  previousdata:PaggedResult<OrderStateDtoModel>=new PaggedResult<OrderStateDtoModel>();

  searchObj :SearchObj= new SearchObj();
  namesearchfield:string=InitialConstants.DefaultString;
  searchfieldsfilter:PaggedCriteriaSearch<OrderStateDtoModel> = new PaggedCriteriaSearch<OrderStateDtoModel>();
  expressionCombinationOperator:ExpressionCombinationOperatorEnum=ExpressionCombinationOperatorEnum.And;

  currentUser:string=InitialConstants.DefaultString;
  ...
  onSubmit(addForm:NgForm){
    ...
    if(addForm.valid){
      if(this.orderState.name.trim().length == InitialConstants.DefaultInt){
        ...
        if(this.orderState.id != InitialConstants.DefaultString){
          this.updateOrderState.id=this.orderState.id;
          this.updateOrderState.name=this.orderState.name;
          this.updateOrderState.lastUpdatedById=this.currentUser;
          this._dataServiceService.orderStateService.update(this.updateOrderState).subscribe((res)=>{
            this._dataServiceService.spinnerService.hide();
          ...
          this.createOrderState.name=this.orderState.name;
          this.createOrderState.createdById=this.currentUser;
          this._dataServiceService.orderStateService.create(this.createOrderState).subscribe((res)=>{
            this._dataServiceService.spinnerService.hide();
            if(res.statusCode==HttpStatusCodes.OK){
  ...
  onEdit(id:string){
    this._dataServiceService.spinnerService.show();
    this._dataServiceService.orderStateService.getById(id).subscribe(res => {
      this._dataServiceService.spinnerService.hide();
      if(res.statusCode==HttpStatusCodes.OK){
        this.onResetPage(InitialConstants.DefaultHidden,TabsValueEnum.Save,TabsTextEnum.Edit);
        this.orderState=res.data ?? new OrderStateDtoModel();
  ...
  onDelete(id:string){
    this._dataServiceService.spinnerService.show();
    this.deleteOrderState.id=id;
    this.deleteOrderState.deletedById=this.currentUser;
    this._dataServiceService.orderStateService.delete(this.deleteOrderState).subscribe((res)=>{
      this._dataServiceService.spinnerService.hide();
      if(res.statusCode==HttpStatusCodes.OK){
  ...
  onResetPage(btn:boolean=InitialConstants.DefaultShow,tabactive:TabsValueEnum=TabsValueEnum.Save,tabname:TabsTextEnum=TabsTextEnum.Save){
    this.orderState=new OrderStateDtoModel();
    this.createOrderState=new CreateOrderStateModel();
    this.updateOrderState=new UpdateOrderStateModel();
    this.deleteOrderState=new DeleteOrderStateModel();
  ...
  onLoadPageSearch(){
    this._dataServiceService.spinnerService.show();
    this._dataServiceService.orderStateService.getPaggedResult(this.searchfieldsfilter).subscribe((res)=>{
      this._dataServiceService.spinnerService.hide();
      if(res.statusCode == HttpStatusCodes.OK){
        this.previousdata=res.data ?? new PaggedResult<OrderStateDtoModel>();
        this.totalpages=PagingInfo.GetTotalPages(res.data?.pagerInfo.totalNumberOfPages ?? InitialConstants.DefaultNoPages);
      }else{
        this._dataServiceService.notificationService.showErrorNotification(res.message);
        this.previousdata=new PaggedResult<OrderStateDtoModel>();
        this.totalpages=[];
      }
    });
  }
