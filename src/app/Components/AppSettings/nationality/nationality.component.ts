...
import {CreateNationalityModel,UpdateNationalityModel,DeleteNationalityModel,NationalityDtoModel} from '../../../Models/Nationality/Nationality.Model';
...
export class NationalityComponent {
  nationality:NationalityDtoModel =new NationalityDtoModel();
  createNationality:CreateNationalityModel=new CreateNationalityModel();
  updateNationality:UpdateNationalityModel=new UpdateNationalityModel();
  deleteNationality:DeleteNationalityModel=new DeleteNationalityModel();
  ...
  previousdata:PaggedResult<NationalityDtoModel>=new PaggedResult<NationalityDtoModel>();

  searchObj :SearchObj= new SearchObj();
  namesearchfield:string=InitialConstants.DefaultString;
  searchfieldsfilter:PaggedCriteriaSearch<NationalityDtoModel> = new PaggedCriteriaSearch<NationalityDtoModel>();
  ...
  onSubmit(addForm:NgForm){
    ...
    if(addForm.valid){
      if(this.nationality.name.trim().length == InitialConstants.DefaultInt){
        ...
        if(this.nationality.id != InitialConstants.DefaultString){
          this.updateNationality.id=this.nationality.id;
          this.updateNationality.name=this.nationality.name;
          this.updateNationality.lastUpdatedById=this.currentUser;
          this._dataServiceService.nationalityService.update(this.updateNationality).subscribe((res)=>{
            this._dataServiceService.spinnerService.hide();
          ...
          this.createNationality.name=this.nationality.name;
          this.createNationality.createdById=this.currentUser;
          this._dataServiceService.nationalityService.create(this.createNationality).subscribe((res)=>{
            this._dataServiceService.spinnerService.hide();
            if(res.statusCode==HttpStatusCodes.OK){
  ...
  onEdit(id:string){
    this._dataServiceService.spinnerService.show();
    this._dataServiceService.nationalityService.getById(id).subscribe(res => {
      this._dataServiceService.spinnerService.hide();
      if(res.statusCode==HttpStatusCodes.OK){
        this.onResetPage(InitialConstants.DefaultHidden,TabsValueEnum.Save,TabsTextEnum.Edit);
        this.nationality=res.data ?? new NationalityDtoModel();
  ...
  onDelete(id:string){
    this._dataServiceService.spinnerService.show();
    this.deleteNationality.id=id;
    this.deleteNationality.deletedById=this.currentUser;
    this._dataServiceService.nationalityService.delete(this.deleteNationality).subscribe((res)=>{
      this._dataServiceService.spinnerService.hide();
      if(res.statusCode==HttpStatusCodes.OK){
  ...
  onOpenDialog(id:string,name:string) {
    ...
    dialogConfig.data = {message:"Do You Want To Delete Nationality "+ name + " ? "};
  ...
  onResetPage(btn:boolean=InitialConstants.DefaultShow,tabactive:TabsValueEnum=TabsValueEnum.Save,tabname:TabsTextEnum=TabsTextEnum.Save){
    this.nationality=new NationalityDtoModel();
    this.createNationality=new CreateNationalityModel();
    this.updateNationality=new UpdateNationalityModel();
    this.deleteNationality=new DeleteNationalityModel();
  ...
  onLoadPageSearch(){
    this._dataServiceService.spinnerService.show();
    this._dataServiceService.nationalityService.getPaggedResult(this.searchfieldsfilter).subscribe((res)=>{
      this._dataServiceService.spinnerService.hide();
      if(res.statusCode == HttpStatusCodes.OK){
        this.previousdata=res.data ?? new PaggedResult<NationalityDtoModel>();
        this.totalpages=PagingInfo.GetTotalPages(res.data?.pagerInfo.totalNumberOfPages ?? InitialConstants.DefaultNoPages);
      }else if (res.statusCode == HttpStatusCodes.NotFound){
        this.previousdata=new PaggedResult<NationalityDtoModel>();
        this.totalpages=[];
      }else{
        this._dataServiceService.notificationService.showErrorNotification(res.message);
        this.previousdata=new PaggedResult<NationalityDtoModel>();
