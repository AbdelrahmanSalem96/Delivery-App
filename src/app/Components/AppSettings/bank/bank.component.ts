...
import {CreateBankModel,UpdateBankModel,DeleteBankModel,BankDtoModel} from '../../../Models/Bank/Bank.Model';
...
export class BankComponent {
  ...
  bank:BankDtoModel =new BankDtoModel();
  createBank:CreateBankModel=new CreateBankModel();
  updateBank:UpdateBankModel=new UpdateBankModel();
  deleteBank:DeleteBankModel=new DeleteBankModel();
  ...
  previousdata:PaggedResult<BankDtoModel>=new PaggedResult<BankDtoModel>();

  searchObj :SearchObj= new SearchObj();
  namesearchfield:string=InitialConstants.DefaultString;
  searchfieldsfilter:PaggedCriteriaSearch<BankDtoModel> = new PaggedCriteriaSearch<BankDtoModel>();
  ...
  onSubmit(addForm:NgForm){
    ...
    if(addForm.valid){
      if(this.bank.name.trim().length == InitialConstants.DefaultInt){
        ...
        if(this.bank.id != InitialConstants.DefaultString){
          this.updateBank.id=this.bank.id;
          this.updateBank.name=this.bank.name;
          this.updateBank.lastUpdatedById=this.currentUser;
          this._dataServiceService.bankService.update(this.updateBank).subscribe((res)=>{
            this._dataServiceService.spinnerService.hide();
          ...
          this.createBank.name=this.bank.name;
          this.createBank.createdById=this.currentUser;
          this._dataServiceService.bankService.create(this.createBank).subscribe((res)=>{
            this._dataServiceService.spinnerService.hide();
            if(res.statusCode==HttpStatusCodes.OK){
  ...
  onEdit(id:string){
    this._dataServiceService.spinnerService.show();
    this._dataServiceService.bankService.getById(id).subscribe(res => {
      this._dataServiceService.spinnerService.hide();
      if(res.statusCode==HttpStatusCodes.OK){
        this.onResetPage(InitialConstants.DefaultHidden,TabsValueEnum.Save,TabsTextEnum.Edit);
        this.bank=res.data ?? new BankDtoModel();
  ...
  onDelete(id:string){
    this._dataServiceService.spinnerService.show();
    this.deleteBank.id=id;
    this.deleteBank.deletedById=this.currentUser;
    this._dataServiceService.bankService.delete(this.deleteBank).subscribe((res)=>{
      this._dataServiceService.spinnerService.hide();
      if(res.statusCode==HttpStatusCodes.OK){
  ...
  onOpenDialog(id:string,name:string) {
    ...
    dialogConfig.data = {message:"Do You Want To Delete Bank "+ name + " ? "};
  ...
  onResetPage(btn:boolean=InitialConstants.DefaultShow,tabactive:TabsValueEnum=TabsValueEnum.Save,tabname:TabsTextEnum=TabsTextEnum.Save){
    this.bank=new BankDtoModel();
    this.createBank=new CreateBankModel();
    this.updateBank=new UpdateBankModel();
    this.deleteBank=new DeleteBankModel();
